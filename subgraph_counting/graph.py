...
class Graph():
    ...
    def __init__(self,
      ...
      edge_attribute_to_label: Callable[[torch.Tensor], str] | None = None
        ...
        if pyg_to_undirected:
            ...
            if edge_attribute is not None:
                (edge_index, edge_attributes) = undirected.to_undirected(pyg_graph.edge_index, pyg_graph[edge_attribute], num_nodes=pyg_graph.num_nodes, reduce=pyg_to_undirected_reduce)
                ...
                edge_index = undirected.to_undirected(pyg_graph.edge_index, num_nodes=pyg_graph.num_nodes, reduce=pyg_to_undirected_reduce)
                duplicate_edge_mask = conversion.get_duplicate_edge_mask(edge_index)
                mask = ~duplicate_edge_mask
                self._edge_index = edge_index[:,mask]
            ...
            self._edge_attributes = pyg_graph[edge_attribute] if edge_attribute is not None else None

        self._num_nodes = pyg_graph.num_nodes # keep all nodes, even if edges are removed
        self._node_attributes = pyg_graph[node_attribute] if node_attribute is not None else None
    ...
    def get_node_colors(self) -> list[int]:
        if self._node_attributes is not None and self._node_colors is None:
            self._node_colors = conversion.to_color(self._node_attributes, self._node_attribute_to_color)

        return self._node_colors

    def get_node_labels(self) -> list[str]:
        if self._node_attributes is not None and self._node_labels is None:
            self._node_labels = conversion.to_label(self._node_attributes, self._node_attribute_to_label)

        return self._node_labels
    
    def get_edge_colors(self) -> list[int]:
        if self._edge_attributes is not None and self._edge_colors is None:
            self._edge_colors = conversion.to_color(self._edge_attributes, self._edge_attribute_to_color)

        return self._edge_colors

    def get_edge_labels(self) -> list[str]:
        if self._edge_attributes is not None and self._edge_labels is None:
            self._edge_labels = conversion.to_label(self._edge_attributes, self._edge_attribute_to_label)
    ...
    def get_edge_indices(self) -> list[Tuple[int, int]]:
        if self._edge_index_list_of_tuples is None:
            ...
            self._edge_index_list_of_tuples = [tuple(edge[:, 0].tolist()) for edge in edges]
    ...
    def count_subisomorphisms(self, subgraph: Graph):
        ...
        subgraph_ig = igraph.Graph(n=subgraph.get_num_nodes(), edges=subgraph.get_edge_indices())
        subisomorphism_count = graph_ig.count_subisomorphisms_vf2(subgraph_ig,
            color1=self.get_node_colors(), color2=subgraph.get_node_colors(),
            edge_color1=self.get_edge_colors(), edge_color2=subgraph.get_edge_colors())
        automorphisms = subgraph_ig.count_automorphisms_vf2(color=subgraph.get_node_colors(), edge_color=subgraph.get_edge_colors())
    ...
    def get_subisomorphisms(self, subgraph: Graph) -> list:
        graph_ig = igraph.Graph(n=self.get_num_nodes(), edges=self.get_edge_indices(), directed=self.is_directed())
        subgraph_ig = igraph.Graph(n=subgraph.get_num_nodes(), edges=subgraph.get_edge_indices(), directed=subgraph.is_directed())
        subisomorphisms = graph_ig.get_subisomorphisms_vf2(subgraph_ig,
            color1=self.get_node_colors(), color2=subgraph.get_node_colors(),
            edge_color1=self.get_edge_colors(), edge_color2=subgraph.get_edge_colors())
    ...
    def plot(self,
      ...
      visual_style: dict | None = None):
        ...
        if not visual_style:
            visual_style = {}
        if node_colors:
            visual_style["vertex_color"] = plot.attributes_to_hexcolor(self.get_node_colors())
        if node_labels:
            visual_style["vertex_label"] = self.get_node_labels()
        if edge_colors:
            visual_style["edge_color"] = plot.attributes_to_hexcolor(self.get_edge_colors())
        if edge_labels:
        ...
        if subgraph is not None:
            ...
            visual_style["vertex_color"] = ['red' if mask else 'white' for mask in sub_nodes]
            visual_style["edge_width"] = [3 if mask else 1 for mask in sub_edges]
        return plot.plot_graph(conversion.graph_to_igraph(self), fname, visual_style)
